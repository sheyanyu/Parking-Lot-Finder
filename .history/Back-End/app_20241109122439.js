...
async function getData(location) {
    try {
        ...
        const data = await parking_lot.find({ _id: location }).toArray();
        return data;
    } catch (error) {
...
app.get('/items', async (req, res) => {
    const { location } = req.query; // Expect location as query parameter
    if (!location) {
        return res.status(400).json({ message: "Location parameter is required" });
    }

    try {
        const data = await getData(location); // Fetch data by location
