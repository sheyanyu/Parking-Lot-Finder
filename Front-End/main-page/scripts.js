    ...
    document.addEventListener("parkingListUpdated", (event) => {
        ...
        parkingList.forEach((parkingLot) => {
            ...
            if (event.target.closest(".parking-lot-info")) {
                ...
                const query = `lat=${parkingLot.location.lat}&lng=${parkingLot.location.lng}&name=${parkingLot.name}&id=${parkingLot.place_id}&distance=${parkingLot.distance}&rate=${parkingLot.rating}`;
